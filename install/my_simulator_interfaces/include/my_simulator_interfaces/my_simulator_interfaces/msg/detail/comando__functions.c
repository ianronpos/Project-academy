// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from my_simulator_interfaces:msg/Comando.idl
// generated code does not contain a copyright notice
#include "my_simulator_interfaces/msg/detail/comando__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
my_simulator_interfaces__msg__Comando__init(my_simulator_interfaces__msg__Comando * msg)
{
  if (!msg) {
    return false;
  }
  // acc
  // delta
  return true;
}

void
my_simulator_interfaces__msg__Comando__fini(my_simulator_interfaces__msg__Comando * msg)
{
  if (!msg) {
    return;
  }
  // acc
  // delta
}

bool
my_simulator_interfaces__msg__Comando__are_equal(const my_simulator_interfaces__msg__Comando * lhs, const my_simulator_interfaces__msg__Comando * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // acc
  if (lhs->acc != rhs->acc) {
    return false;
  }
  // delta
  if (lhs->delta != rhs->delta) {
    return false;
  }
  return true;
}

bool
my_simulator_interfaces__msg__Comando__copy(
  const my_simulator_interfaces__msg__Comando * input,
  my_simulator_interfaces__msg__Comando * output)
{
  if (!input || !output) {
    return false;
  }
  // acc
  output->acc = input->acc;
  // delta
  output->delta = input->delta;
  return true;
}

my_simulator_interfaces__msg__Comando *
my_simulator_interfaces__msg__Comando__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_simulator_interfaces__msg__Comando * msg = (my_simulator_interfaces__msg__Comando *)allocator.allocate(sizeof(my_simulator_interfaces__msg__Comando), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(my_simulator_interfaces__msg__Comando));
  bool success = my_simulator_interfaces__msg__Comando__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
my_simulator_interfaces__msg__Comando__destroy(my_simulator_interfaces__msg__Comando * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    my_simulator_interfaces__msg__Comando__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
my_simulator_interfaces__msg__Comando__Sequence__init(my_simulator_interfaces__msg__Comando__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_simulator_interfaces__msg__Comando * data = NULL;

  if (size) {
    data = (my_simulator_interfaces__msg__Comando *)allocator.zero_allocate(size, sizeof(my_simulator_interfaces__msg__Comando), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = my_simulator_interfaces__msg__Comando__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        my_simulator_interfaces__msg__Comando__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
my_simulator_interfaces__msg__Comando__Sequence__fini(my_simulator_interfaces__msg__Comando__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      my_simulator_interfaces__msg__Comando__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

my_simulator_interfaces__msg__Comando__Sequence *
my_simulator_interfaces__msg__Comando__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_simulator_interfaces__msg__Comando__Sequence * array = (my_simulator_interfaces__msg__Comando__Sequence *)allocator.allocate(sizeof(my_simulator_interfaces__msg__Comando__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = my_simulator_interfaces__msg__Comando__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
my_simulator_interfaces__msg__Comando__Sequence__destroy(my_simulator_interfaces__msg__Comando__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    my_simulator_interfaces__msg__Comando__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
my_simulator_interfaces__msg__Comando__Sequence__are_equal(const my_simulator_interfaces__msg__Comando__Sequence * lhs, const my_simulator_interfaces__msg__Comando__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!my_simulator_interfaces__msg__Comando__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
my_simulator_interfaces__msg__Comando__Sequence__copy(
  const my_simulator_interfaces__msg__Comando__Sequence * input,
  my_simulator_interfaces__msg__Comando__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(my_simulator_interfaces__msg__Comando);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    my_simulator_interfaces__msg__Comando * data =
      (my_simulator_interfaces__msg__Comando *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_simulator_interfaces__msg__Comando__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          my_simulator_interfaces__msg__Comando__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!my_simulator_interfaces__msg__Comando__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
